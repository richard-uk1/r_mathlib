...
#[no_mangle]
pub unsafe extern "C" fn log1pexp(mut x: libc::c_double) -> libc::c_double {
    if x <= 18.0f64 {
        return Rlog1p(exp(x));
    }
    if x > 33.3f64 {
...
#[no_mangle]
pub unsafe extern "C" fn log1mexp(mut x: libc::c_double) -> libc::c_double {
    return if -x > -0.693147180559945309417232121458f64 {
        log(-expm1(-x))
    } else {
        Rlog1p(-exp(-x))
    };
...
pub unsafe extern "C" fn plogis(
    ...
    if log_p != 0 {
        return -log1pexp(if lower_tail != 0 { -x } else { x });
    } else {
        return 1 as libc::c_int as libc::c_double
            / (1 as libc::c_int as libc::c_double + exp(if lower_tail != 0 { -x } else { x }));
    };
}
