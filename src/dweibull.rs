...
#[no_mangle]
pub unsafe extern "C" fn dweibull(
    mut x: libc::c_double,
    ...
    if x == 0 as libc::c_int as libc::c_double && shape < 1 as libc::c_int as libc::c_double {
        return 1.0f64 / 0.0f64;
    }
    tmp1 = pow(x / scale, shape - 1 as libc::c_int as libc::c_double);
    tmp2 = tmp1 * (x / scale);
    return if log_p != 0 {
